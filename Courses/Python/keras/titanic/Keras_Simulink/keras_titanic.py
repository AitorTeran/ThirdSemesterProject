...
history = model.fit(
        ...
        epochs = 100,
        callbacks = [callbacks]) #fit(x=None, y=None, batch_size=None, epochs=1, verbose=1, callbacks=None, validation_split=0.0, validation_data=None, shuffle=True, class_weight=None, sample_weight=None, initial_epoch=0, steps_per_epoch=None, validation_steps=None, validation_freq=1, max_queue_size=10, workers=1, use_multiprocessing=False)

model.save_weights(".\weights.h5", save_format = "h5")
# Run Tensorboard by navigating to the project folder in CMD and running the command >tensorboard --logdir ".\Tensorboard_logs". Open browser and go to localhost:6006
...
) # Probably needs to have pydot and graphviz installed. Use conda for both packages. Graphviz also needs to have its .msi installed and the dot.exe file added to system path. Refer to google
